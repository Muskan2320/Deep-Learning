...
def data_preprocessing(task_1a_dataframe):
    ...
    encoded_dataframe['Education'] = encoded_dataframe['Education'].astype('category')
    encoded_dataframe['Education'] = encoded_dataframe['Education'].cat.codes
    # encoded_dataframe.drop(['Education'], axis=1)
    
    encoded_dataframe['City'] = encoded_dataframe['City'].astype('category')
    encoded_dataframe['City'] = encoded_dataframe['City'].cat.codes
    
    encoded_dataframe['Gender'] = encoded_dataframe['Gender'].astype('category')
    encoded_dataframe['Gender'] = encoded_dataframe['Gender'].cat.codes
    
    encoded_dataframe['EverBenched'] = encoded_dataframe['EverBenched'].astype('category')
    encoded_dataframe['EverBenched'] = encoded_dataframe['EverBenched'].cat.codes
...
def identify_features_and_targets(encoded_dataframe):
    ...
    features_and_targets.append(encoded_dataframe.iloc[:, [0,2,3,5,6,7]])
    features_and_targets.append(list(encoded_dataframe.iloc[:, 8]))
	##########################################################
...
def load_as_tensors(features_and_targets):
	...
	#################	ADD YOUR CODE HERE	##################
    X_train, X_test, Y_train, Y_test = train_test_split(features_and_targets[0], features_and_targets[1], test_size = 0.25)

    tensors_and_iterable_training_data = []
    tensors_and_iterable_training_data.append(torch.tensor(X_train.values).float())
    tensors_and_iterable_training_data.append(torch.tensor(X_test.values).float())
    tensors_and_iterable_training_data.append(torch.tensor(Y_train).float())
    tensors_and_iterable_training_data.append(torch.tensor(Y_test).float())
    #     X = torch.tensor(features_and_targets[0].values).float()
    # Y = torch.tensor(features_and_targets[1]).float()
    itr_object = DataLoader(list(zip(torch.tensor(features_and_targets[0].values).float(), torch.tensor(features_and_targets[1]).float())), batch_size = 32)
    tensors_and_iterable_training_data.append(itr_object)
	##########################################################
...
class Salary_Predictor(nn.Module):
    ...
    def __init__(self):
		...
		'''
		#######	ADD YOUR CODE HERE	#######
        self.Matrix1 = nn.Linear(6, 36)
        self.activation = nn.ReLU()
        self.norm_layer1 = torch.nn.BatchNorm1d(36)
        self.Matrix5 = nn.Linear(36, 1)
        self.T = nn.Tanh()
        self.S = nn.Sigmoid()
		###################################	
    ...
    def forward(self, x):
		...
		'''
		#######	ADD YOUR CODE HERE	#######
        x = self.T(self.Matrix1(x))
        x = self.activation(x)
        ##x = self.T(self.Matrix4(x))
        x = self.S(self.Matrix5(x))
...
def training_function(model, number_of_epochs, tensors_and_iterable_training_data, loss_function, optimizer):
    ...
    for _ in range(number_of_epochs):
        for X_Batch, Y_Batch in tensors_and_iterable_training_data[4]:
            optimizer.zero_grad()
            ypred = model(X_Batch)
            loss_value = loss_function(ypred, Y_Batch)
            loss_value.backward()
            optimizer.step()
...
def validation_function(trained_model, tensors_and_iterable_training_data):
	...
	#################	ADD YOUR CODE HERE	##################
    model.eval()
    y_pred = model(tensors_and_iterable_training_data[1])
    accuracy = (y_pred.round() == tensors_and_iterable_training_data[3]).float().mean()
